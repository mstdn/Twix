...
<template>
    <AppLayout title="Home">
                ...
                <div class="flex">
                    <div class="w-full">
                        <div class="flex items-center">
                            ...
                            <Link v-if="$page.props.auth.user !== null" :href="route('add.reply', { post: post.data })"
                                        ...
                                        <PopoverPanel
                                            ...
                                            <div
                                                ...
                                                <div class="bg-gray-50 p-2">
                                                    <Link v-if="post.data.delete === true"
                                                        @click="destroy(post.data.id)" method="post" type="submit" as="button"
        ...
        <div v-if="replies.total !== 0" v-for="reply in replies.data" :key="reply.id"
            class="border-b border-l border-r border-gray-200 dark:border-dim-200 hover:bg-gray-100 dark:hover:bg-dim-300 cursor-pointer transition duration-350 ease-in-out pb-4">
            <div class="flex flex-shrink-0 p-4 pb-0">
                <a href="#" class="flex-shrink-0 group block">
                    <div class="flex items-top">
                        <div>
                            <img class="inline-block h-9 w-9 rounded-full" :src="reply.user.avatar" alt="" />
                        </div>
                        <div class="ml-3">
                            <p class="flex items-center text-base leading-6 font-medium text-gray-800 dark:text-white">
                                <Link :href="route('user.show', { id: reply.user.username })">
                                {{ reply.user.name }}
                                </Link>
                                <svg viewBox="0 0 24 24" aria-label="Verified account" fill="currentColor"
                                    class="w-4 h-4 ml-1 text-blue-500 dark:text-white">
                                ...
                                <span
                                    class="ml-1 text-sm leading-5 font-medium text-gray-400 group-hover:text-gray-300 transition ease-in-out duration-150">
                                    <Link :href="route('user.show', { id: reply.user.username })">@{{ reply.user.username }}</Link> ·
                                    <Link :href="route('post.show', { user: reply.user.username, post: reply.id })">{{
                                        reply.time }}</Link>
                                </span>
                            </p>
                        </div>
            ...
            <div class="pl-16">
                ...
                <div v-if="reply.media.length" class="carousel rounded-box my-3 mr-2 rounded-2xl">
                    <div v-for="(item, index) in reply.media" :key="index" class="carousel-item ">
                        <img class="rounded-2xl object-cover h-full w-full max-h-[300px]" :src="item.full_url" />
                    </div>
                </div>

                <div v-if="reply.video.length" class="flex mb-4 mr-2">
                    <div v-for="(item, index) in reply.video" :key="index">
                        <vue-plyr :options="options">
                            <video controls crossorigin playsinline loop>
                                <source size="720" :src="item.full_url" type="video/mp4" />
                            </video>
                ...
                <div class="flex">
                    <div class="w-full">
                        <div class="flex items-center">
                            ...
                            <Link v-if="$page.props.auth.user !== null" :href="route('add.reply', { post: reply })"
                                class="flex-1 flex items-center text-gray-800 dark:text-white text-xs text-gray-400 hover:text-blue-400 dark:hover:text-blue-400 transition duration-350 ease-in-out">
                                <svg viewBox="0 0 24 24" fill="currentColor" class="w-5 h-5 mr-2">
                                    <g>
                                        <path
                            ...
                            <Link v-if="$page.props.user !== null && reply.isliked === false" preserveScroll method="post"
                                as="button" class="
                                flex-1 flex
                                items-center
                                text-gray-800
                            ...
                            <Link v-if="$page.props.user !== null && reply.isliked === true" preserveScroll method="post"
                                as="button" class="
                                flex-1 flex
                                items-center
                                text-red-600
